...
if len(standings) != 6:
	raise ValueError("Incorrect number of players in standings after registration.")
...
if not already_played(tournament,playerIDs[0],playerIDs[1]):
	raise ValueError("Function already_played returned FALSE for players who played together.")
if already_played(tournament,playerIDs[0],playerIDs[3]):
	raise ValueError("Function already_played returned TRUE for players who haven't played with each other yet.")
...
for i in range(len(standings)):
	...
	if standings[i][2] != expected_score[i]:
		raise ValueError("Returned standings contain incorrect score for players")
	if standings[i][7] != 2:
